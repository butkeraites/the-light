...
use the_light_core::userdata::plans::{available_plans, plan_by_id, Plan, PlanProgress};
use the_light_core::userdata::PlanStore;
...
fn active() -> std::result::Result<(PlanStore, PlanProgress, Plan), ExitCode> {
    ...
    let progress = match st.load() {
        Ok(Some(p)) => p,
        Ok(None) => {
            eprintln!("Nenhum plano ativo. Use `light plan start <id>`.");
...
fn ics(output: Option<&Path>) -> ExitCode {
    ...
    line("VERSION:2.0");
    line("PRODID:-//The Light//Reading Plan//PT");
    ...
    match output {
        Some(path) => {
            if let Err(e) = the_light_core::util::atomic_write(path, ics.as_bytes()) {
