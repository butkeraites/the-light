...
//!
//! Layout (sob o diretório de dados do SO, ou `BIBLIA_DATA_DIR`):
...
/// Diretório base de dados do usuário (`BIBLIA_DATA_DIR` tem prioridade).
///
/// Linux: `~/.local/share/biblia/`; macOS: `~/Library/Application Support/biblia/`.
pub fn data_dir() -> Result<PathBuf> {
    if let Some(p) = std::env::var_os("BIBLIA_DATA_DIR") {
        return Ok(PathBuf::from(p));
    }
    let dirs = directories::ProjectDirs::from("", "", "biblia").ok_or(UserDataError::NoDataDir)?;
