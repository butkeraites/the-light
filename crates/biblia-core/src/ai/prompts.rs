...
//! `prompts/<slug>.md` no diretório de config (ou em `BIBLIA_PROMPTS`); se
...
/// Diretório de prompts override (`BIBLIA_PROMPTS` ou `<config>/prompts`).
pub fn prompts_dir() -> Result<PathBuf> {
    if let Some(p) = std::env::var_os("BIBLIA_PROMPTS") {
