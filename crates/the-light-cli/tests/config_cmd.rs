//! Testes de integração do comando `light config` (via `LIGHT_CONFIG`).
...
/// Comando `light` com `LIGHT_CONFIG`/`LIGHT_SECRETS` em arquivos temporários
/// no mesmo diretório do `cfg`.
fn light(cfg: &std::path::Path) -> Command {
    let mut cmd = Command::cargo_bin("light").unwrap();
    cmd.env("LIGHT_CONFIG", cfg);
    let secrets = cfg.parent().unwrap().join("secrets.toml");
    cmd.env("LIGHT_SECRETS", secrets);
    cmd
}
