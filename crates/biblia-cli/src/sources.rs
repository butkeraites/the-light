...
use biblia_core::model::{License, Translation, TranslationId};
use biblia_core::source::{ApiBibleSource, BibleSource, EmbeddedSource, EsvApiSource};
    ...
    match c.kind.as_str() {
        "apibible" => {
            let key = key.ok_or_else(|| {
                "conector API.Bible sem chave — use `biblia config set-key apibible <chave>`"
                    .to_string()
            })?;
        ...
        "esv" => {
            let key = key.ok_or_else(|| {
                "conector ESV sem chave — use `biblia config set-key esv <chave>`".to_string()
            })?;
