...
//! pode ser sobrescrito por `LIGHT_SECRETS` (testes/cenários avançados).
//!
...
impl KeyStore {
    /// Caminho do arquivo de segredos (`LIGHT_SECRETS` tem prioridade).
    pub fn secrets_path() -> Result<PathBuf> {
        if let Some(p) = std::env::var_os("LIGHT_SECRETS") {
