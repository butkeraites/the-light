...
use the_light_core::config::{Config, Connector};
...
#[derive(Subcommand)]
enum ConfigAction {
    /// Define um valor: `light config set versions kjv,alm1911`.
    Set {
    ...
    /// Lê um valor: `light config get versions`.
    Get {
        /// Chave a consultar.
        key: String,
    },
...
fn connector_add(
    ...
    let language = match the_light_core::model::Lang::from_str(&lang) {
        Ok(l) => l,
        Err(e) => {
    ...
    println!("Defina a chave com: light config set-key {kind} <chave>");
