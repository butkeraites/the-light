//! Testes de integração do comando `light read` (via `assert_cmd`).
//!
...
fn light() -> Command {
    ...
    cmd.env(
        "LIGHT_CONFIG",
        std::env::temp_dir().join("light_absent_config.toml"),
    );
    cmd.env(
        "LIGHT_DATA_DIR",
        std::env::temp_dir().join("light_absent_data_dir"),
    );
    cmd.env(
        "LIGHT_SECRETS",
        std::env::temp_dir().join("light_absent_secrets.toml"),
    );
    cmd
}
