...
impl Store {
    ...
    pub fn default_db_path() -> Result<PathBuf> {
        let dirs = directories::ProjectDirs::from("", "", "biblia").ok_or(StoreError::NoDataDir)?;
...
fn ensure_fts5(conn: &Connection) -> Result<()> {
    let probe = conn.execute_batch(
        "CREATE VIRTUAL TABLE IF NOT EXISTS temp.__biblia_fts_probe USING fts5(x);\
         DROP TABLE temp.__biblia_fts_probe;",
    );
    match probe {
        Ok(()) => Ok(()),
